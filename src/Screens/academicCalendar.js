...
const Academic = ({ navigation }) => {
  ...
  const [futureDates, setFutureDates] = useState(
    academicCalendar.filter((date) => isInTheFuture(new Date(date.start_date)))
  );
  ...
  function onUpdate() {
    dispatch(updateAcademicCalendar(selectedDate));
  }
  useEffect(() => {
    setFutureDates(
      academicCalendar.filter((date) =>
                ...
                return (
                  ...
                  >
                    <List.Item
                      ...
                      right={(props) => (
                        <>
                          {user && user.additionalPrivilage === 1 ? (
                            <Button
                              variant="outline"
                              title="Update"
