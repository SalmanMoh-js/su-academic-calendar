...
const Budget = ({ navigation }) => {
  const { user, budgetCalendar, loading, errors, dataUpdated, addDataLoading } =
  ...
  const [futureDates, setFutureDates] = useState(
    budgetCalendar.filter((date) => isInTheFuture(new Date(date.start_date)))
  );
  ...
  function onUpdate() {
    dispatch(updateBudgetCalendar(selectedDate));
  }
  useEffect(() => {
    setFutureDates(
      budgetCalendar.filter((date) => isInTheFuture(new Date(date.start_date)))
                ...
                return (
                    ...
                    right={(props) => (
                      <View className="flex flex-row">
                        {futureDatesId.indexOf(item.id) === -1 ? null : (
                          <Badge
                            label={`${daysLeft(item.start_date)} days left`}
                            color="primary"
                          />
                        )}
                        <Badge
                  ...
                  >
                    <List.Item
                      ...
                      right={(props) => (
                        <>
                          {user && user.additionalPrivilage === 2 ? (
                            <Button
                              variant="outline"
                              title="Update"
